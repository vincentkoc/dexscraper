...
from .models import TradingPair, PriceData, LiquidityData, VolumeData
...
def decode_pair(data: bytes) -> Optional[TradingPair]:
    ...
    try:
        ...
        if 'solana' in printable or any(proto in printable.lower() for proto in ['pump', 'raydium']):
        ...
        for field_idx, field in enumerate(fields):
            ...
            if str_len > min(200, len(data) - pos):
                logger.debug(f"Suspicious length {str_len} for field {field} at pos {pos}")
        ...
        if (price_data and (price_data.current != 0 or price_data.usd != 0)) or \
           (volume_data and volume_data.h24 != 0) or \
           (liquidity_data and liquidity_data.usd != 0):
            return trading_pair
...
def decode_pair_from_text(data: bytes) -> Optional[TradingPair]:
    ...
    try:
        ...
        for word in words:
            ...
            if any(proto in word_clean.lower() for proto in ['pumpswap', 'raydium', 'meteora', 'jupiter', 'orca']):
                protocol = word_clean
            # Long addresses (Solana addresses are typically 32-44 chars, base58)
            elif len(word_clean) >= 32 and len(word_clean) <= 44 and word_clean.replace('1', '').isalnum():
                if not token_address:
                    token_address = word_clean
                elif not pair_address:
                    pair_address = word_clean
            # Token symbols (short, often uppercase)
            elif word_clean.isupper() and 2 <= len(word_clean) <= 10 and word_clean.isalpha():
                if not token_symbol:
                    token_symbol = word_clean
            # Token names (longer descriptive text)
            elif 3 <= len(word_clean) <= 50 and not word_clean.isnumeric() and not word_clean.startswith('http'):
                # Prefer longer, more descriptive names
                if not token_name or len(word_clean) > len(token_name):
                    # Avoid obvious non-names
                    if not any(skip in word_clean.lower() for skip in ['twitter', 'telegram', 'website', 'pump', 'sol']):
    ...
    try:
        ...
        for i, section in enumerate(sections[1:], 1):  # Skip first empty section
            ...
            for word in words:
                ...
                if any(proto in word.lower() for proto in ['pump', 'raydium', 'meteora', 'jupiter']):
                    protocol = word
                # Potential addresses (long alphanumeric strings)
                elif len(word) >= 32 and len(word) <= 44 and word.replace('1', '').isalnum():
                    if not token_address:
                        token_address = word
                    elif not pair_address:
                        pair_address = word
...
def parse_message(message: bytes) -> List[TradingPair]:
    ...
    try:
        ...
        logger.debug(f"Starting pair parsing at position {pos}, message length: {len(message)}")
