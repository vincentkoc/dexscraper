...
def log_extraction_success(batch_size: int, high_confidence: int, duration_ms: float):
    ...
    logger = get_logger()
    logger.info(f"Extraction successful: {batch_size} tokens, {high_confidence} high-confidence ({duration_ms:.2f}ms)")
...
def log_token_profile_built(symbol: str, confidence: float, field_count: int):
    ...
    logger = get_logger()
    logger.debug(f"Built profile: {symbol} (confidence: {confidence:.0%}, fields: {field_count})")