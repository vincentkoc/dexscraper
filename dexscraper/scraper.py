...
class DexScraper:
    ...
    async def _extract_all_tokens(self, data: bytes, data_start: int) -> List[TokenProfile]:
        ...
        logger.debug("Using validated deep analysis approach from analyze_protocol_deep.py...")
        ...
        for pos in positions:
            ...
            token_record = self._extract_validated_token_record(token_name, record_data, pos)
            if token_record:
    ...
    def _extract_real_token_names(self, printable: str, data_start: int) -> Dict[int, str]:
        ...
        blacklist = {'HTTP', 'HTTPS', 'GET', 'POST', 'PUT', 'DELETE', 'JSON', 
                    'XML', 'HTML', 'CSS', 'JS', 'API', 'URL', 'URI', 'TCP', 
                    'UDP', 'DNS', 'SSL', 'TLS', 'UTF', 'ASCII', 'BASE', 
                    'TRUE', 'FALSE', 'NULL', 'VOID', 'INT', 'FLOAT', 'DOUBLE'}
        ...
        english_words = {'The', 'And', 'For', 'Are', 'But', 'Not', 'You', 'All',
                        ...
                        'Http', 'Https', 'Json', 'Server', 'Client', 'Request',
                        'Response', 'Error', 'Success', 'Failed', 'Retry'}
    ...
    def _extract_validated_token_record(self, token_name: str, record_data: bytes, position: int) -> Optional[TokenProfile]:
        ...
        if len(fields) >= 3:
            if self.debug:
                logger.debug(f"✅ Complete record for {token_name} with {len(fields)} fields: {list(fields.keys())}")
    ...
    def _extract_token_symbols(self, text: str, data_start: int) -> List[Dict]:
        ...
        blacklist = {'HTTP', 'HTTPS', 'API', 'GET', 'POST', 'PUT', 'DELETE', 
                    'JSON', 'XML', 'HTML', 'CSS', 'JS', 'PNG', 'JPG', 'GIF',
                    'TRUE', 'FALSE', 'NULL', 'VOID', 'INT', 'CHAR', 'BOOL', 'STRING',
                    'AND', 'OR', 'NOT', 'IF', 'ELSE', 'FOR', 'WHILE', 'DO', 'RETURN',
                    'CONST', 'VAR', 'LET', 'NEW', 'THIS', 'SUPER', 'TRY', 'CATCH',
                    'URL', 'URI', 'TCP', 'UDP', 'DNS', 'SSL', 'TLS', 'UTF', 'ASCII',
                    'BASE', 'ERROR', 'INFO', 'WARN', 'DEBUG', 'TRACE'}
        
        # Whitelist known good token symbols from our analysis
        whitelist = {'SOL', 'USDC', 'USD', 'WLFI', 'COIN', 'MROCKS', 'OTC', 
                    'KAIROS', 'EMULITES', 'LYN', 'FINANCE', 'SHOT', 'HOPE',
                    'AA', 'EA', 'FA', 'PEPE', 'DOGE', 'SHIB', 'BONK'}
        ...
        symbol_context_pattern = re.compile(r'(?:symbol|name|token)[:\s]*([A-Za-z0-9]{2,10})', re.IGNORECASE)
        ...
        for match in set(mixed_matches):
            # Filter out common English words
            if match.lower() not in ['the', 'and', 'for', 'are', 'but', 'not', 'you', 'all', 
                                   'can', 'had', 'her', 'was', 'one', 'our', 'out', 'day', 
                                   'get', 'has', 'him', 'his', 'how', 'its', 'may', 'new',
                                   'now', 'old', 'see', 'two', 'way', 'who', 'boy', 'did',
                                   'does', 'each', 'find', 'here', 'just', 'like', 'long',
                                   'make', 'many', 'over', 'part', 'some', 'time', 'very',
                                   'what', 'with', 'have', 'from', 'they', 'know', 'want',
                                   'been', 'good', 'much', 'some', 'time', 'very', 'when',
                                   'come', 'could', 'state', 'there', 'think', 'where',
                                   'will', 'would', 'about', 'after', 'again', 'below',
                                   'being', 'both', 'could', 'every', 'first', 'found',
                                   'great', 'group', 'large', 'last', 'left', 'life',
                                   'little', 'never', 'next', 'often', 'other', 'own',
                                   'right', 'small', 'still', 'such', 'take', 'than',
                                   'them', 'well', 'were', 'Http', 'Https', 'Json']:
    ...
    def _extract_best_token_symbol(self, metadata: Dict, index: int) -> str:
        ...
        for token in tokens:
            ...
            if 2 <= len(symbol) <= 10 and symbol.replace('_', '').replace('-', '').isalnum():
    ...
    def _group_clusters_to_tokens(self, clusters: List[Dict], metadata: Dict) -> List[Dict]:
        ...
        for cluster in clusters[:20]:  # Process top 20 clusters
            ...
            relevant_metadata = {'addresses': [], 'urls': [], 'protocols': [], 'age_indicators': [], 'tokens': []}
