...
from dexscraper.models import TokenProfile, ExtractedTokenBatch, OHLCData
from dexscraper.utils import (
    extract_floats_from_bytes, extract_doubles_from_bytes,
    extract_solana_addresses, extract_urls, cluster_numeric_values,
    is_valid_float, validate_trading_data
)
...
class TestBinaryProtocolEdgeCases:
    ...
    def test_ui_data_validation(self):
        ...
        for price, volume, txns, makers, liquidity, market_cap, expected_valid in test_cases:
            is_valid = validate_trading_data(price, volume)
            if expected_valid:
                assert is_valid, f"Should be valid: price={price}, volume={volume}"
            else:
                assert not is_valid, f"Should be invalid: price={price}, volume={volume}"
    ...
    def test_url_extraction_patterns(self):
        ...
        for url in test_urls:
            ...
            assert any(url.split('/')[-1] in ext_url for ext_url in extracted), f"Should extract URL component from: {url}"
