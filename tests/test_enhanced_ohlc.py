...
from dexscraper import DexScraper, TokenProfile, ExtractedTokenBatch
...
async def test_enhanced_ohlc_output():
    ...
    for token in top_tokens:
        ohlc = token.to_ohlc()
        if ohlc:
            print(f"{token.get_display_name()},{ohlc.timestamp},{ohlc.open:.8f},{ohlc.high:.8f},{ohlc.low:.8f},{ohlc.close:.8f},{ohlc.volume:.0f}")
            ohlc_count += 1
    ...
    for i, ohlc in enumerate(batch_ohlc[:5]):
        print(f"  {i+1}: {datetime.fromtimestamp(ohlc.timestamp).strftime('%H:%M:%S')} | "
    ...
    for pair in trading_pairs[:5]:
        ohlc = pair.to_ohlc()
        if ohlc:
            print(f"  {pair.base_token_symbol}: ${ohlc.close:.8f} | Vol: ${ohlc.volume:,.0f}")
            legacy_ohlc_count += 1
        ...
        if batch.tokens:
            ...
            for token in top_3:
                ...
                if ohlc:
                    ...
                    print(f"  {token.get_display_name():<12} | ${ohlc.close:<12.8f} | {confidence:<4} | {fields}")
...
async def test_enhanced_streaming():
    ...
    def streaming_callback(batch):
        ...
        extraction_count += 1
        print(f"📡 Stream {extraction_count}: {batch.total_extracted} tokens, "
        ...
        for token in batch.get_top_tokens(3):
            ohlc = token.to_ohlc()
            if ohlc:
                print(f"   {token.get_display_name()}: ${ohlc.close:.8f} | Vol: ${ohlc.volume:,.0f}")
    
    print("Testing streaming with callback for 3 cycles...")
    
    # Simulate streaming for a short time
    try:
        task = asyncio.create_task(scraper.stream_pairs(
